...
export default function Navbar() {
  ...
  const menuItems = [
    ...
    { name: 'Shop Detail', href: '/products/sample' },
    {
      name: 'Pages',
      href: '#',
      hasDropdown: true,
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          ...
          <Link href="/" className="flex items-center space-x-2">
            <div className="text-green-600 font-bold text-3xl">Fruitables</div>
          </Link>
                  ...
                  <div className="absolute top-full left-0 mt-2 w-48 bg-white rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                    {item.subItems.map((subItem) => (
                      <Link
                        key={subItem.name}
                        href={subItem.href}
                        className="block px-4 py-3 text-gray-700 hover:bg-green-50 hover:text-green-600 first:rounded-t-lg last:rounded-b-lg transition-colors"
                      >
                        {subItem.name}
                      </Link>
                    ))}
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link
              href="/cart"
              className="hidden md:block text-gray-700 hover:text-green-600 font-semibold"
            >
              $0.00
            </Link>
              ...
              item.hasDropdown ? (
                <div key={item.name} className="py-2">
                  <button
                    ...
                    className="flex items-center justify-between w-full px-4 py-2 text-gray-700 hover:text-green-600 font-medium transition-colors"
                  >
                    <span>{item.name}</span>
                    <FiChevronDown
